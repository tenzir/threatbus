...
class MISP:
    def __init__(self, config):
        ...
        if config.zmq:
            ...
            self.logger.debug("launching Kafka with "
                              f"{json.dumps(config.kafka.config)}")
    ...
    def propose_removal_of_ids_flag(self, attr_id):
        ...
        self.logger.debug("proposing to remove IDS flag "
                          f"from attribute {attr_id}")
    ...
    async def snapshot(self):
        ...
        assert data
        print(data)
        return [MISP.make_intel_from_attribute(x) for x in data["response"]["Attribute"]]
